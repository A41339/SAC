using System.Collections.Generic;
using System.ServiceModel;
using System.Threading.Tasks;
using Entities.Entities.Evaluacion;
using FGA.Models;

// NOTE: You can use the "Rename" command on the "Refactor" menu to change the interface name "IEntidades" in both code and config file together.
[ServiceContract]
public interface IEvalCategoriaService
{
    [OperationContract]
    [ReferencePreservingDataContractFormat]
    Task<List<EvalCategoria>> GetByUser(int idUser);

    [OperationContract]
    [ReferencePreservingDataContractFormat]
    Task<List<EvalCategoria>> GetAllAsync();
    
    [OperationContract]
    [ReferencePreservingDataContractFormat]
    EvalCategoria Get(string id);

    [OperationContract]
    void Add(ref EvalCategoria entity);

    [OperationContract]
    void Update(EvalCategoria entity);

    [OperationContract]
    void Delete(string id);

}
